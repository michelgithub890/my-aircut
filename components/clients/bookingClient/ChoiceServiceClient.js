    ...
    return (
        <div>
            ...
            {lists?.sort((a,b) => a.name.localeCompare(b.name)).map(list => (
                ...
                <div key={list.id} className="mt-3 p-2 mx-2" style={{ border:`1px solid ${MODEL_COLOR[valueColor]}`}}>
                    <div 
                    ...
                    {showList === list.id && 
                        <>
                            ...
                            {services.filter(service => service.idList === list.id).map(service => (
                                <div key={service.id} className="p-3" onClick={() => _handleChoiceService(service)} style={{ cursor:"pointer", borderTop:`1px solid ${MODEL_COLOR[valueColor]}` }}>
